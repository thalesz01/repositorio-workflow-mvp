...
| `WorkFlowProject.Infrastructure` | Persistência com Dapper/SQL Server, fábrica de conexões e implementações dos executores de nodes. |
| `WorkFlowProject.API` | API ASP.NET Core que expõe operações para criar, consultar e iniciar workflows. |
| `WorkFlowProject.Worker` | Serviço em segundo plano que localiza execuções pendentes e processa uma etapa por vez. |
...
O script `src/WorkFlowProject.Infrastructure/Scripts/CreateTables.sql` cria as tabelas abaixo.
...
4. Inicie o Worker `WorkFlowProject.Worker`.
